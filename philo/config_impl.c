...
/*   Updated: 2023/06/30 05:20:17 by tjaasalo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	config_time_to_die(t_config *self, const char *arg)
{
	...
	if (!self->is_valid)
		return ;
	errstr = NULL;
	self->time_to_die = ft_strtonum(arg, 0, int_max(), &errstr);
	if (!errstr)
		return ;
...
void	config_time_to_eat(t_config *self, const char *arg)
{
	...
	if (!self->is_valid)
		return ;
	errstr = NULL;
	self->time_to_eat = ft_strtonum(arg, 0, int_max(), &errstr);
	if (!errstr)
		return ;
...
void	config_time_to_sleep(t_config *self, const char *arg)
{
	...
	if (!self->is_valid)
		return ;
	errstr = NULL;
	self->time_to_sleep = ft_strtonum(arg, 0, int_max(), &errstr);
	if (!errstr)
		return ;
